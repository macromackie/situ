...
function buildCodexWorkspaceManagerPrompt(input: {
  ...
  return [
    ...
    "",
    "Overfit discipline:",
    "- Treat held-out/final metrics as a two-sided generalization check, not as another target to tune.",
    "- Do not read protected held-out data directly, and do not build lookup tables or training data from dev-set labels.",
    "- If a candidate reaches a near-perfect dev metric but held-out/final remains materially lower, mark it as overfit-risky rather than a clean accepted result.",
    "- A dev-trained or lookup-style branch may be useful as a dev frontier datapoint, but the final report must call out the risk and separately identify the best non-leaky generalizing branch.",
    "- Unless the user explicitly says the objective is dev-only, do not move an overfit-risky candidate to accepted without a clear caveat in reviews, live records, and the report.",
    "",
    "Delegation guidance:",
...
function judgeFacetLines(workspaceCase: WorkspaceAutoresearchCase): readonly string[] {
  const commonFacets = [
    ...
    "- `overfit-discipline`: Did the manager protect held-out/final data and avoid presenting a suspicious dev-set frontier, dev-label lookup, or large dev-vs-heldout gap as a clean accepted generalizing result?",
